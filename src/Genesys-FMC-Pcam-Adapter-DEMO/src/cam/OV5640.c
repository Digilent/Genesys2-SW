...
{
	uint8_t id_h[1], id_l[1];
	id_h[0] = 0;
	id_l[0] = 0;
	readReg(IicInstance, reg_ID_h, &id_h[0], channel_mask_);
	readReg(IicInstance, reg_ID_l, &id_l[0], channel_mask_);

	if (id_h[0] != dev_ID_h_ || id_l[0] != dev_ID_l_)
	{
		xil_printf("\r\nGot %02x %02x. Expected %02x %02x\r\n", id_h[0], id_l[0], dev_ID_h_, dev_ID_l_);
	...
	writeReg(IicInstance, 0x3103, 0x11, channel_mask_);
	//[7]=1 Software reset; [6]=1 Software power down; Default=0x02
	// We apply software reset
	writeReg(IicInstance, 0x3008, 0x82, channel_mask_);
	...
	{
		writeReg(IicInstance, cfg_init_[i].addr, cfg_init_[i].data, channel_mask_);
	}
...
Errc set_mode(XIic *IicInstance, cam_mode_t mode, uint8_t channel_mask_)
{
	...
	//[7]=1 Software reset; [6]=1 Software power down; Default=0x02
	// We apply software power down
	//	writeReg(IicInstance, 0x3008, 0x42, channel_mask_);

	config_modes_t const cfg_mode = modes[mode];
	writeConfig(IicInstance, cfg_mode.cfg, cfg_mode.cfg_size, channel_mask_);

	  //[7]=1 Software reset; [6]=1 Software power down; Default=0x02
	 // We power up the camera
	//	writeReg(IicInstance, 0x3008, 0x02, channel_mask_);
	return OK;
}
...
Errc set_awb(XIic *IicInstance, awb_t awb, uint8_t channel_mask_)
{
   ...
   //writeReg(IicInstance, 0x3008, 0x42, channel_mask_);
	...
	config_awb_t const cfg_mode = awbs[awb];
	writeConfig(IicInstance, cfg_mode.cfg, cfg_mode.cfg_size, channel_mask_);
...
Errc set_isp_format(XIic *IicInstance, isp_format_t isp, uint8_t channel_mask_)
{
	...
	{
	case ISP_RGB:
		writeReg(IicInstance, OV5640_FORMAT_MUX_CONTROL, 0x01, channel_mask_);
		break;
	case ISP_RAW:
		writeReg(IicInstance, OV5640_FORMAT_MUX_CONTROL, 0x03, channel_mask_);
		break;
	default:
		break;
	}
...
void set_test(XIic *IicInstance, test_t test, uint8_t channel_mask_)
{
	...
	{
	case TEST_DISABLED:
		writeReg(IicInstance, OV5640_REG_PRE_ISP_TEST_SET1, 0x00, channel_mask_);
		break;
	case TEST_EIGHT_COLOR_BAR:
		writeReg(IicInstance, OV5640_REG_PRE_ISP_TEST_SET1, 0x80, channel_mask_);
		break;
	default:
		break;
	}
...
{
			uint8_t read_buf[2];
			read_buf[0] = (uint8_t) (reg_addr >> 8);
			read_buf[1] = (uint8_t) (reg_addr);
			mux_write(IicInstance, dev_address1_, read_buf, 2, channel_mask_);
			mux_read(IicInstance, dev_address1_, buf, 1, channel_mask_);
}

void writeReg(XIic *IicInstance, uint16_t reg_addr, uint8_t reg_data, uint8_t channel_mask_)
{
//			auto buf = std::vector<uint8_t>{(uint8_t)(reg_addr>>8), (uint8_t)reg_addr, reg_data};
			...
			mux_write(IicInstance, dev_address1_, buf, 3, channel_mask_);
...
//			auto buf = std::vector<uint8_t>{reg_data};
			...
			mux_write(IicInstance, dev_address2_, buf, SIZEOF_ARRAY(buf), channel_mask_);
}
...
void writeConfig(XIic *IicInstance, config_word_t const* cfg, size_t cfg_size, uint8_t channel_mask_)
{
	...
	{
		writeReg(IicInstance, cfg[i].addr, cfg[i].data, channel_mask_);
	}
}
